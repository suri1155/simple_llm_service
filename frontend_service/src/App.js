...
export default function LLMQueryApp() {
  ...
  function handleAuthError(message) {
    // mark session expired and clear auth state
  ...
  const fetchStats = async (authToken) => {
    try {
      ...
      if (response.ok) {
        ...
        const bodyMessage = (data && (data.detail || data.message || data.error || '')) || '';
        if (typeof bodyMessage === 'string' && bodyMessage.toLowerCase().includes('invalid token')) {
          handleAuthError('Session expired. Please log in again.');
          return;
        }
        setStats(data);
  ...
  const fetchHistory = async () => {
    try {
      ...
      if (!response.ok) {
        ...
        setError(errBody.detail || errBody.message || 'Failed to fetch history');
        return;
      }
      ...
      items.forEach(item => {
        formattedMessages.push({
          role: 'user',
          content: item.query || item.question || item.prompt || 'No query text',
          timestamp: item.timestamp || item.created_at
        });
        formattedMessages.push({
          role: 'assistant',
          content: item.response || item.answer || item.result || 'No response',
          timestamp: item.timestamp || item.created_at
        });
      });
